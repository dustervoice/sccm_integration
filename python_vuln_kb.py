...
#get the cvrf data and extract kb's for the CVE of interest
def get_knowledge_bases_for_cve(cve):
    ...
    response = requests.get(url, headers = headers)
    data = json.loads(response.content)
    kbs = {'KB{}'.format(kb['Description']['Value']) for vuln in data["Vulnerability"] if vuln["CVE"] == cve for kb in vuln["Remediations"] if kb['SubType'] != 'Monthly Rollup'}
    return list(kbs)
